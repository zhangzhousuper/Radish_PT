...
bool init() {
  ...
  window = glfwCreateWindow(width, height, "Radish PT", NULL, NULL);
  if (!window) {
    glfwTerminate();
    return false;
  }
  ...
  ImGui::Begin("Options");
  {
    ...
    if (Settings::denoiser == Denoiser::EAWavelet) {
      const char *Modes[] = {
          ...
          "Motion",        "Input Direct",    "Input Indirect",
          "Output Direct", "Output Indirect", "Composed"};
      ImGui::Combo("Preview", &Settings::ImagePreviewOpt, Modes,
                   IM_ARRAYSIZE(Modes));

      ImGui::SliderInt("Levels", &EAWFilter.level, 1, 5);
      ImGui::DragFloat("Sigma Lumin", &EAWFilter.waveletFilter.sigLumin, .01f,
                       0.f);
      ImGui::DragFloat("Sigma Normal", &EAWFilter.waveletFilter.sigNormal, .01f,
                       0.f);
      ImGui::DragFloat("Sigma Depth", &EAWFilter.waveletFilter.sigDepth, .01f,
                       0.f);
      ...
      const char *Modes[] = {
          ...
          "Motion",           "Input Direct",    "Input Indirect",
          "Output Direct",    "Output Indirect", "Composed",
          "Direct Moment",    "Indirect Moment", "Direct Variance",
          "Indirect Variance"};
      ImGui::Combo("Preview", &Settings::ImagePreviewOpt, Modes,
                   IM_ARRAYSIZE(Modes));

      ImGui::SliderInt("Levels", &directFilter.level, 1, 5);
      ImGui::DragFloat("Sigma Lumin", &directFilter.waveletFilter.sigLumin,
                       .01f, 0.f);
      ImGui::DragFloat("Sigma Normal", &directFilter.waveletFilter.sigNormal,
                       .01f, 0.f);
      ImGui::DragFloat("Sigma Depth", &directFilter.waveletFilter.sigDepth,
                       .01f, 0.f);

      indirectFilter.level = directFilter.level;
      indirectFilter.waveletFilter.sigLumin =
          directFilter.waveletFilter.sigLumin;
      indirectFilter.waveletFilter.sigNormal =
          directFilter.waveletFilter.sigNormal;
      indirectFilter.waveletFilter.sigDepth =
          directFilter.waveletFilter.sigDepth;
    }
...
void mainLoop() {
  while (!glfwWindowShouldClose(window)) {
    ...
    string title = "Radish | " + utilityCore::convertIntToString(iteration) +
                   " Iterations";
