...
struct AABB {
  ...
  __host__ __device__ glm::vec3 center() const { return (pMin + pMax) * .5f; }
  ...
  __host__ __device__ bool intersect(Ray ray, float &tMin) {
    ...
    if (glm::abs(dir.x) > 1.f - Eps) {
      ...
      if (Math::between(ori.z, pMin.z, pMax.z) &&
          Math::between(ori.x, pMin.x, pMax.x)) {
      ...
      if (Math::between(ori.x, pMin.x, pMax.x) &&
          Math::between(ori.y, pMin.y, pMax.y)) {
    ...
    if (tDist.y + tDist.z > yz && tDist.z + tDist.x > zx &&
        tDist.x + tDist.y > xy) {
      return getDistMaxMin(fmaxf(tNear.x, tNear.y), tNear.z,
