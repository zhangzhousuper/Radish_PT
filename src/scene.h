...
struct DevScene {
  ...
  __device__ glm::vec3 proceduralTexture(glm::vec2 uv) {
    thrust::default_random_engine rng(int(uv.x * 1024) * 1024 +
                                      int(uv.y * 1024));
    float rx = thrust::uniform_real_distribution<float>(0.f, 1.f)(rng);
    float ry = thrust::uniform_real_distribution<float>(0.f, 1.f)(rng);

    float f = (glm::sin(uv.x * 10.f * TWO_PI + rx * TWO_PI) + +1.f) * .5f;
    float g = (glm::sin(uv.y * 10.f * TWO_PI + ry * TWO_PI) + +1.f) * .5f;
    return glm::vec3(f * g);
  }
  ...
  __device__ Material getTexturedMaterialAndSurface(Intersection &intersec) {
    ...
    if (mat.baseColorMapId != NullTextureId) {
      mat.baseColor =
          mat.baseColorMapId == ProceduralTexId
              ? proceduralTexture(intersec.uv)
              : textures[mat.baseColorMapId].linearSample(intersec.uv);
    }
  ...
  __device__ float environmentMapPdf(glm::vec3 wi) {
    glm::vec3 radiance = envMap->linearSample(Math::toPlane(wi));
    return Math::luminance(radiance) * sumLightPowerInv * envMap->width *
           envMap->height * 0.5f;
  }

  __device__ float sampleEnvironmentMap(glm::vec3 pos, glm::vec2 r,
    ...
    wi = Math::toSphere(
        glm::vec2((x + 0.5f) / envMap->width, (y + 0.5f) / envMap->height));
    ...
    return Math::luminance(radiance) * sumLightPowerInv * envMap->width *
           envMap->height * INV_PI * INV_PI * 0.5f;
  }

  __device__ float sampleEnvMapNoVisbility(glm::vec3 pos, glm::vec2 r,
                                           glm::vec3 &radiance, glm::vec3 &wi) {
    int pixId = envMapSampler.sample(r.x, r.y);
  ...
  __device__ float sampleDirectLightNoVisibility(glm::vec3 pos, glm::vec4 r,
                                                 glm::vec3 &radiance,
                                                 glm::vec3 &wi, float &dist) {
    if (lightSampler.length == 0) {
      return INVALID_PDF;
    }
    int lightId = lightSampler.sample(r.x, r.y);
